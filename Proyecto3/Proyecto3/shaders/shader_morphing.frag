...
layout(set=0, binding = 4) uniform sampler2D BaseTex1;

layout(set=0, binding = 5) uniform sampler2D BaseTex2;
...
void main() 
{
  vec3 TexColor = vec3( mix(texture(BaseTex1,TexCoord), texture(BaseTex2,TexCoord), Time.time));
